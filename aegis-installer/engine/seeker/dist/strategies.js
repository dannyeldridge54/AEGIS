...
function mutateParams(base, params, magnitude, rng) {
    const result = {};
    for (const p of params) {
...
function crossover(a, b, params, rng) {
    const result = {};
    for (const p of params)
        result[p.name] = rng.random() < 0.5 ? a[p.name] : b[p.name];
    return result;
}
/**
 ...
 */
function gradientStep(base, params, history, rng, learningRate = 0.05) {
    if (history.length < 3)
 ...
 */
function surrogateStep(params, history, rng) {
    if (history.length < params.length * 3)
        return randomParams(params, rng);
    ...
    for (const p of params) {
        ...
        if (Math.abs(denom) < 1e-15) {
            ...
            const a = (sy * sx2 * sx4 + sx * sx3 * sx2y + sx2 * sxy * sx3
                - sx2 * sx2 * sx2y - sx * sy * sx4 - sxy * sx3 * n) / denom;
            const b = (n * sxy * sx4 + sy * sx3 * sx2 + sx2 * sx * sx2y
                - sx2 * sxy * sx2 - sy * sx * sx4 - n * sx2y * sx3) / denom;
            if (a > 1e-10) {
                // Minimum at x = -b / (2a)
 ...
 */
function noveltySample(params, history, rng, resolution = 20) {
    const bins = new Map();
...
// ─── Generate Next Point ─────────────────────────────────────────────────────
function generateNextPoint(strategy, params, best, history, rng, constraints, evalCount, gridIndex) {
    const maxAttempts = 50;
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        let candidate;
        switch (strategy.type) {
            case 'grid':
                candidate = gridSample(params, strategy.config.resolution || 20, gridIndex || Math.floor(rng.random() * 10000));
                break;
            case 'random':
            ...
            case 'evolutionary':
                if (best && history.length > 2) {
                    ...
                    candidate = mutateParams(crossover(best.params, parent2.params, params, rng), params, strategy.config.mutationRate || 0.1, rng);
                }
                else {
                    candidate = randomParams(params, rng);
                }
