...
async function compareModels(config) {
    ...
    for (const benchId of benchmarkIds) {
        ...
        if (!bench) {
            if (verbosity !== 'silent')
                console.log(`⚠️ Unknown benchmark: ${benchId}, skipping`);
            continue;
        }
        if (verbosity !== 'silent') {
            console.log(`\n── ${bench.name} ──`);
        ...
        for (let trial = 0; trial < config.trials; trial++) {
            ...
            for (const model of config.models) {
                const agent = new agent_1.SeekerAgent(bench, {
                    ...model.agentConfig,
...
function formatReport(leaderboard, pairwise, config) {
    const lines = [
        '',
        '╔═══════════════════════════════════════════════════════════╗',
        '║                   LEADERBOARD                             ║',
    ...
    for (const pw of pairwise) {
        ...
        const dir = pw.test.direction === 'a_better' ? `${pw.modelA} >`
            : pw.test.direction === 'b_better' ? `${pw.modelB} >`
                : 'tied';
        lines.push(`║  ${pw.modelA} vs ${pw.modelB}: ${sig} p=${pw.test.pValue.toFixed(4)} d=${pw.test.effectSize.toFixed(2)} ${dir} ║`);
    }
    lines.push('╚═══════════════════════════════════════════════════════════╝');
    return lines.join('\n');
}
