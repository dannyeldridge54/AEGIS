...
exports.CodingAgent = exports.Sandbox = exports.OllamaProvider = exports.OpenAIProvider = void 0;
...
class Sandbox {
    ...
    async execute(code, language = 'typescript') {
        if (!this.allowExec) {
...
class CodingAgent {
    ...
    async generate(task) {
        ...
        if (files.length > 0) {
            for (const f of files)
                this.sandbox.writeFile(f.path, f.content);
        }
        else {
    ...
    async evolveStrategy(problemDescription, currentBestStrategy, performanceData) {
...
`;
    ...
    // ─── Internal ────────────────────────────────────────────────────────────
    async autoFix(code, error, task, lang) {
