...
exports.EquationWriter = void 0;
exports.createEquationWriter = createEquationWriter;
...
class EquationWriter {
    constructor(outputDir = './discoveries') {
        this.discoveries = [];
        this.bestByDomain = new Map();
    ...
    writeDiscovery(engine, taskId, result, score) {
    ...
    synthesizeEinsteinCartan(engine, result, score) {
    ...
    synthesizeFTGravity(engine, result, score) {
        const p = result.params;
    ...
    synthesizeUFETorsion(engine, result, score) {
        const p = result.params;
    ...
    synthesizeTorsionWave(engine, result, score) {
        const p = result.params;
        const mu2 = p.mu2;
        ...
        let gwDetector;
        if (f_Hz > 10)
            gwDetector = 'LIGO/Virgo/KAGRA (10-1000 Hz)';
        else if (f_Hz > 1e-4)
            gwDetector = 'LISA (0.1 mHz - 0.1 Hz)';
        else if (f_Hz > 1e-9)
            gwDetector = 'Pulsar Timing Arrays (nHz)';
        else
            gwDetector = 'CMB B-mode polarization';
        const targets = [];
        // GW detector targets
        targets.push({
    ...
    synthesizeCrossDomain(engine, result, score) {
        const p = result.params;
        const mu2 = p.mu2 || 0;
...
σ (spin density) = ${p.spinDensity.toExponential(4)} J·s/m³`;
    ...
    ftPlaintext(p, modelType, modelName) {
...
Cosmological fit: χ²/d.o.f. against 21 H(z) data points`;
    ...
    ufePlaintext(p, T_vev, m_T, charLength) {
...
Field equation: -2μ²T + 4λT³ + γ□T + 2εRT + κ_f·<ψ̄γ⁵ψ> = 0`;
    ...
    wavePlaintext(p, effMass, T_vev, v_group) {
...
Cross-sector couplings:
  ...
  EC coupling: ${(p.ec_coupling || 0).toFixed(4)}`;
    ...
    // ── Helpers ─────────────────────────────────────────────────────────────
    classifyDomain(taskId) {
        if (taskId.includes('einstein-cartan'))
            return 'einstein-cartan';
        if (taskId.includes('ft-gravity'))
            return 'ft-gravity';
        if (taskId.includes('ufe-torsion'))
            return 'ufe-torsion';
        if (taskId.includes('torsion-wave'))
            return 'torsion-wave';
        if (taskId.includes('cross-domain'))
            return 'cross-domain';
        return null;
    }
    findSpinSources(spinDensity) {
        // Return sources whose estimated spin density is within a few orders of magnitude
        if (spinDensity < 1e30)
            return SPIN_SOURCES.filter(s => s.type.includes('pulsar'));
        if (spinDensity < 1e38)
            return SPIN_SOURCES.filter(s => s.type.includes('pulsar') || s.type.includes('magnetar'));
        return SPIN_SOURCES;
    }
    computeFTDeviations(p) {
        const H0 = 67.4;
        const OmegaM = 0.315;
        const observations = [
    ...
    predictH0(p) {
        ...
        switch (modelType) {
            case 0:
                fT = p.alpha * Math.pow(Math.abs(T0), p.n);
                break;
            case 1:
                fT = Math.abs(p.lambda) * (Math.sqrt(1 + 2 * T0 / Math.abs(p.lambda)) - 1);
                break;
            case 2:
                fT = p.alpha * T0 + p.beta * T0 * Math.log(Math.abs(T0 / (-6 * H0_base * H0_base)));
                break;
            case 3:
                fT = p.alpha * T0 * (1 - Math.exp(p.beta));
                break;
            default: fT = 0;
        }
...
exports.EquationWriter = EquationWriter;
// ─── Factory ─────────────────────────────────────────────────────────────────
function createEquationWriter(outputDir) {
    return new EquationWriter(outputDir);
}
//# sourceMappingURL=equation-writer.js.map