...
exports.torsionTasks = exports.crossDomainTask = exports.torsionWaveTask = exports.ufeTorsionTask = exports.fTGravityTask = exports.einsteinCartanTask = exports.HZ_OBSERVATIONS = void 0;
exports.computeTorsionScalar = computeTorsionScalar;
exports.computeTraceVector = computeTraceVector;
exports.computeAxialTorsion = computeAxialTorsion;
exports.einsteinCartanResidual = einsteinCartanResidual;
exports.fTGravity_PowerLaw = fTGravity_PowerLaw;
exports.fTGravity_BornInfeld = fTGravity_BornInfeld;
exports.fTGravity_Logarithmic = fTGravity_Logarithmic;
exports.fTGravity_Exponential = fTGravity_Exponential;
exports.computeSpatialAnomalies = computeSpatialAnomalies;
exports.fTCosmologyResidual = fTCosmologyResidual;
exports.ufeTorsionFunctional = ufeTorsionFunctional;
exports.crossDomainUFE = crossDomainUFE;
exports.torsionWaveResidual = torsionWaveResidual;
 ...
 */
function computeTorsionScalar(components) {
...
function computeTraceVector(components, dim = 4) {
...
function computeAxialTorsion(components) {
 ...
 */
function einsteinCartanResidual(params) {
    const { T01, T02, T03, T12, T13, T23, // 6 independent torsion components
 ...
 */
function fTGravity_PowerLaw(T, alpha, n) {
    return alpha * Math.pow(Math.abs(T), n) * Math.sign(T);
}
function fTGravity_BornInfeld(T, lambda) {
    const arg = 1 + 2 * T / lambda;
    if (arg <= 0)
        return lambda * (-1); // saturate when argument goes negative
    return lambda * (Math.sqrt(arg) - 1);
}
function fTGravity_Logarithmic(T, alpha, beta, T0) {
    if (Math.abs(T) < 1e-30)
        return 0;
    return alpha * T + beta * T * Math.log(Math.abs(T / T0));
}
function fTGravity_Exponential(T, alpha, beta, T0) {
    if (Math.abs(T) < 1e-30)
        return 0;
    return alpha * T * (1 - Math.exp(beta * T0 / T));
}
exports.HZ_OBSERVATIONS = [
    {
...
function computeSpatialAnomalies(params) {
    const { alpha, beta, n, lambda, modelType } = params;
    const H0 = 67.4;
    const OmegaM = 0.315;
    const anomalies = [];
    for (const obs of exports.HZ_OBSERVATIONS) {
        ...
        switch (mType) {
            case 0:
                fT = fTGravity_PowerLaw(T, alpha, n);
                break;
            case 1:
                fT = fTGravity_BornInfeld(T, Math.abs(lambda) || 1);
                break;
            case 2:
                fT = fTGravity_Logarithmic(T, alpha, beta, -6 * H0 * H0);
                break;
            case 3:
                fT = fTGravity_Exponential(T, alpha, beta, -6 * H0 * H0);
                break;
 ...
 */
function fTCosmologyResidual(params) {
    const { alpha, beta, n, lambda, modelType } = params;
    const H0 = 67.4;
    const OmegaM = 0.315;
    let chi2 = 0;
 ...
 */
function ufeTorsionFunctional(params) {
    const { mu2, // μ² — mass² parameter (positive for symmetry breaking)
    ...
    if (Math.abs(kappa_f) > 4 * Math.PI)
        penalty += 10 * (Math.abs(kappa_f) - 4 * Math.PI) ** 2;
 ...
 */
function crossDomainUFE(params) {
    const { 
 ...
 */
function torsionWaveResidual(params) {
    const { mu2, // Mexican hat mass² parameter
...
exports.crossDomainTask = {
    id: 'ufe-cross-domain',
    name: 'UFE Cross-Domain Unified v3',
    evaluate: crossDomainUFE,
    parameters: [
        { name: 'T_scalar', min: 1e-10, max: 100, description: 'Torsion scalar magnitude (forced non-zero)' },
        { name: 'mu2', min: 1e-3, max: 1e15, description: 'μ² mass parameter (positive for SSB)' },
        { name: 'lambda_quartic', min: 1e-3, max: 50, description: 'Quartic self-coupling (positive)' },
        { name: 'spinDensity', min: 1, max: 1e20, description: 'Spin source (non-zero)' },
        { name: 'ec_coupling', min: -50, max: 50, description: 'EC torsion-curvature coupling' },
        { name: 'fT_alpha', min: -20, max: 20, description: 'f(T) amplitude' },
        { name: 'fT_n', min: 0.1, max: 4, description: 'f(T) power law index' },
        { name: 'wave_freq', min: 0.01, max: 5000, description: 'Torsion wave frequency' },
        { name: 'wave_source', min: -1000, max: 1000, description: 'Wave spin current source' },
        { name: 'H0_rescaled', min: 0.85, max: 1.15, description: 'H₀ in units of 70 km/s/Mpc' },
    ],
};
/** All torsion tasks bundled */
exports.torsionTasks = {
    einsteinCartan: exports.einsteinCartanTask,
    fTGravity: exports.fTGravityTask,
    ufeTorsion: exports.ufeTorsionTask,
    torsionWave: exports.torsionWaveTask,
    crossDomain: exports.crossDomainTask,
};
//# sourceMappingURL=torsion.js.map