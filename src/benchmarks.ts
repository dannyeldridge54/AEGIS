...
import { parallelOptimize } from './parallel';
import { Task, EvalResult } from './interfaces';
...
export const benchmarkFunctions = {
  ...
  rosenbrock: {
    ...
    evaluate: (p: Record<string, number>) => Math.pow(1 - p.x, 2) + 100 * Math.pow(p.y - p.x * p.x, 2),
    parameters: [{ name: 'x', min: -5, max: 5 }, { name: 'y', min: -5, max: 5 }],
    optimum: 0, optimumParams: { x: 1, y: 1 },
  } as Task & { optimum: number; optimumParams: Record<string, number> },
  ...
  sphere5d: {
    ...
    evaluate: (p: Record<string, number>) => Object.values(p).reduce((s, v) => s + v * v, 0),
...
export interface BenchmarkResult {
  ...
  score: number;
  optimum: number;
  gap: number;      // score - optimum (lower = better)
  accuracy: number; // 1 - gap/optimum (higher = better, max 1)
  evals: number;
  time: number;
...
export async function runBenchmarks(config?: {
  ...
  for (const name of funcNames) {
    const fn = (benchmarkFunctions as any)[name];
    if (!fn) continue;

    const start = Date.now();
    const result = await optimize(fn.evaluate, fn.parameters, {
      maxEvals,
      verbosity: 'silent',
    });
