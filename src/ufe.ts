...
export class UFETracker {
  ...
  private computeTimeToTarget(optimum?: number): UFEMetrics['timeToTarget'] {
    ...
    for (const [evalIdx, bestScore] of this.curve) {
      if (result.pct10 === null && (this.minimize ? bestScore <= targets.pct10 : bestScore >= targets.pct10)) result.pct10 = evalIdx;
      if (result.pct50 === null && (this.minimize ? bestScore <= targets.pct50 : bestScore >= targets.pct50)) result.pct50 = evalIdx;
      if (result.pct90 === null && (this.minimize ? bestScore <= targets.pct90 : bestScore >= targets.pct90)) result.pct90 = evalIdx;
    }
    return result;
  }
