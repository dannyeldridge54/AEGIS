...
export const finance = {
  ...
  async getPrice(symbol: string): Promise<{ price: number; change: number; volume: number } | null> {
    try {
...
export const environment = {
  ...
  async getWeather(lat: number, lon: number): Promise<{
    temperature: number; windSpeed: number; humidity: number; description: string;
  } | null> {
    try {
...
export const engineering = {
  ...
  atmosphere(altitude_m: number): { temperature_K: number; pressure_Pa: number; density_kg_m3: number } {
...
export const math = {
  ...
  reducedChi2(observed: number[], expected: number[], errors: number[], nParams: number): number {
    ...
    return math.chi2(observed, expected, errors) / dof;
  },
};
...
export const live = {
  ...
  async earthquakes(minMagnitude: number = 4.5): Promise<Array<{
  ...
  /** Fetch ISS position */
  async issPosition(): Promise<{ lat: number; lon: number; altitude: number; velocity: number } | null> {
    try {
  ...
  async solarActivity(): Promise<{ sunspots: number; flux: number; kp: number } | null> {
    try {
  ...
  async arxivRecent(category: string = 'astro-ph.CO', maxResults: number = 5): Promise<Array<{
    title: string; authors: string; summary: string; published: string;
  }>> {
    try {
