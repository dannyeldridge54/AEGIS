...
export class SeekerAgent {
  ...
  async run(optimum?: number): Promise<AgentState> {
    ...
    if (this.config.persistence.enabled) this.saveState();
  ...
  async step(): Promise<EvalResult> {
    ...
    const params = generateNextPoint(
      strategy, this.task.parameters, this.state.best,
      this.state.history, this.rng,
      this.task.constraints, this.state.totalEvals, this.gridIndex++
    );
  ...
  getSeed(): number { return this.rng.seed; }
  ...
  private checkConvergence(): boolean {
    if (this.state.totalEvals < 200 || !this.state.best) return false;
    ...
    if (olderSlice.length === 0) return false;
...
export function seeker(task: Task, config?: AgentConfig): SeekerAgent {
