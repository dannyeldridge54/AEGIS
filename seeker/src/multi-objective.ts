...
import { ParameterDef, EvalResult, UFEMetrics } from './interfaces';
import { MetaLearner, generateNextPoint } from './strategies';
import { SeededRNG } from './rng';
import { UFETracker } from './ufe';
...
function computeCrowding(solutions: ParetoResult[], objectives: string[]): void {
  ...
  if (n <= 2) { solutions.forEach(s => s.crowdingDistance = Infinity); return; }
...
export async function multiOptimize(
  ...
  const rng = new SeededRNG(options?.seed || undefined);
  const ufeTracker = new UFETracker(parameters, true);
  ...
  while (totalEvals < maxEvals) {
    ...
    for (let i = 0; i < batchSize; i++) {
      ...
      batch.push(generateNextPoint(strategy, parameters, best, history as EvalResult[], rng));
    }
    ...
    for (const params of batch) {
      ...
      const scalar = objectives.reduce((sum, obj) =>
        sum + scores[obj.name] * (obj.weight || 1) * (obj.minimize !== false ? 1 : -1), 0);

      const evalResult: EvalResult = { params, score: scalar, timestamp: Date.now(), strategy: strategy.type };
