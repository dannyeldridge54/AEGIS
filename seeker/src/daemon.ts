...
interface JobState {
  ...
  ufeRatio: number;
  discoveries: number;
  history: Array<{ timestamp: number; score: number; evals: number; ufeRatio: number }>;
}

export class SeekerDaemon {
  ...
  private activeAgents: Map<string, SeekerAgent> = new Map();
  ...
  constructor(config?: DaemonConfig) {
    ...
    if (!fs.existsSync(this.config.workDir)) fs.mkdirSync(this.config.workDir, { recursive: true });
  ...
  pauseJob(id: string): void {
    ...
    state.status = 'paused';
    const agent = this.activeAgents.get(id);
    if (agent) { agent.stop('Paused'); this.activeAgents.delete(id); }
  }

  resumeJob(id: string): void {
    const state = this.jobStates.get(id);
    if (state && state.status === 'paused') { state.status = 'queued'; state.nextRun = Date.now(); }
  }

  getJobStates(): JobState[] { return [...this.jobStates.values()]; }
  ...
  async start(): Promise<void> {
    ...
    if (this.config.apiPort > 0) this.startAPI();
  ...
  stop(): void {
    ...
    for (const [, agent] of this.activeAgents) agent.stop('Daemon shutdown');
  ...
  private async runJob(jobId: string): Promise<void> {
    ...
    if (this.config.enableMemory) {
      ...
      if (suggested) this.log(`  Memory suggests: ${suggested} strategy`);
    }

    const agent = new SeekerAgent(job.task, {
    ...
    this.server = http.createServer((req, res) => {
      ...
      if (url === '/api/status') {
        ...
        for (const [id, state] of this.jobStates) {
          if (state.best) bests[id] = { score: state.best.score, params: state.best.params, ufeRatio: state.ufeRatio };
        }
...
export function startDaemon(name: string, jobs: DaemonJob[], config?: Partial<DaemonConfig>): SeekerDaemon {
